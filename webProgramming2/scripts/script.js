...
$(document).ready(function () {
    ...
    const slider = $('#slider');
    let images = slider.find('img').toArray();
    ...
    async function moveRight() {  //一つ右の画像に移動する
        if (!clickable) return
        clickable = false;
        //ボタンの色変化
    ...
    async function moveLeft() {   //一つ左の画像に移動する
        if (!clickable) return;
        clickable = false;
        //ボタンの色変化
        $(buttons[nowIndex]).css('background-color', '#8F8F8F');
    ...
    $(slider).on('touchend', async function () {
        if (!clickable) return
        const def = endX - startX;
        if (def < -offset() / 2) {
            moveRight();
        } else if (def > offset() / 2) {
            moveLeft();
        } else {
            clickable = false;
            const newMoveTime = moveTime * Math.abs(def) / offset() * 2;
            await Promise.all(
                $(images).map((_, element) => {
                    return new Promise(resolve => {
